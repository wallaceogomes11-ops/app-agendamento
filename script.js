...
function init() {
  atualizarRelogio();
  atualizarDataHeader();
  renderizarDias();        // renderiza os 7 dias futuros
  renderizarSlots();       // renderiza horários do dia selecionado
  bindEventos();
...
function selecionarDia(data, btn) {
  ...
  // Atualiza o resumo do modal com a nova data
  dom.summaryData.textContent = chave;

  // Re-renderiza os horários do dia selecionado
  renderizarSlots();
}
