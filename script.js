...
function aplicarCabecalho() {
  ...
  if (cfg.LOGO_FILENAME) {
    const ext      = cfg.LOGO_FILENAME.split('.').pop();
    const logoSrc  = 'assets/logo.' + ext;
    const brandIcon = document.querySelector('.brand-icon');
    if (brandIcon) {
      brandIcon.innerHTML = '<img src="' + logoSrc + '" alt="Logo" style="width:100%;height:100%;object-fit:cover;border-radius:6px;" onerror="this.style.display=\'none\';" />';
    }
  }
}
